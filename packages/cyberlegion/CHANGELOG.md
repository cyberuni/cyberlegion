...
- 7cb6aaa: Take `cyber-mux` 0.5.0, whose `read` answers with `{ text, truncated? }` instead of a bare
...
it and begin work."`) because the peer's brief was injected into its context by the peer's own
  ...
  The doorbell is now the instruction itself, naming the brief's **file path**: _read your brief at
  `<path>`, then begin work_. The brief is still written to its file and still never typed into the
  ...
  - `--no-wake` changed meaning without changing its flag: previously _no turn, brief auto-loaded_; now
    _no turn and brief unread on disk_. A caller driving the first turn itself must convey the path.
