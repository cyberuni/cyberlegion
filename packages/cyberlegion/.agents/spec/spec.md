...
approval:
  spec:
    ...
    why:
      ...
      blast: medium — spec + suite only at this gate, confined to two nodes of one project spec. Measured against the files rather than estimated: `mail/doorbell` was in the intake estimate and is NOT touched (no scenario there references the spawn doorbell — the node is `mail send` delivery-ring scoped). The implementation this contract admits is larger and is what the impl gate judges.
      ...
      confidence: high — three cold spec-judge rounds, each finding something the previous missed. R1 short-circuited on a governance pre-flight miss (producer declared 0 of 7 bars). R2 returned builder FAIL on a real discrimination defect: the replacement `mail/surface` scenario was a barred edge with no firing-direction companion — its `Given` pinned the brief file but not the peer's registry status, so under the only constructible fixture (`active`) the un-retired `inject-inbox.ts:40` branch is inert and a partial implementer passed every `Then`. R3 verified the chosen remedy kills that subject two independent ways and re-derived R2's fixes independently rather than deferring to them.
      judge: cold sdd-spec-judge round 3 — oracle/builder/architect all PASS; ALIGNED true; SCENARIOS_FAILING []; 24/24 scenarios clean in `mail/surface`, 51/51 in `unit/lifecycle`; no blocker, no open questions.
