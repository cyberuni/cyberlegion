...
> **Version pin.** Invocations read `npx cyberlegion@0.3.0 …` as a placeholder. Until the package
...
```bash
npx cyberlegion@0.3.0 unit register --standing            # lists the standing owner record(s)
```
...
```bash
npx cyberlegion@0.3.0 mail inbox --owner <handle>            # all owner mail, oldest-first
npx cyberlegion@0.3.0 mail inbox --owner <handle> --unread   # only what is new
```
...
```bash
npx cyberlegion@0.3.0 mail read <msg-id> --owner <handle>
```
...
```bash
npx cyberlegion@0.3.0 mail ack <msg-id> --owner <handle>
```
...
```bash
npx cyberlegion@0.3.0 mail send --to <agent-or-thread-origin> --thread <t> --body "<answer>"
```
