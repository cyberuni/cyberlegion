...
```bash
npx cyberlegion@0.3.0 agent resolve <R> --format json
```
...
```bash
npx cyberlegion@0.3.0 mux doctor --format json
```
...
- **channel** — warm, interactive, and a pane is available: spin up a live peer that can converse
  ...
  ```bash
  npx cyberlegion@0.3.0 unit spawn --agent <R> --brief-file <B> --at <placement>
  npx cyberlegion@0.3.0 mail await --thread <the-minted-thread-id> [--timeout <ms>] [--max-wait <s>]
  ```
