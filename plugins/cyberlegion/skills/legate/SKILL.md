...
> **Version pin.** Every invocation below reads `npx cyberlegion@0.3.0 ...` — a placeholder.
...
|---|---|
| Send a message to a named peer, "the pane on the right", a claude/cursor peer | `npx cyberlegion@0.3.0 mail send --to <handle>` (or `unit nudge <ref>` first if the peer needs waking, then `mail send`) |
| Check inbox / read unread mail | `npx cyberlegion@0.3.0 mail inbox` — `mail read <msg-id>` to peek, `mail ack <msg-id>` once handled |
| Spawn a new peer session | `npx cyberlegion@0.3.0 unit spawn --agent <name> ...` — cyberlegion owns worktree creation; pass `--cwd <dir>` only when the caller already has a directory to spawn into (e.g. reusing an existing worktree) |
| Close / tear down a peer session | `npx cyberlegion@0.3.0 unit close <id>` |
| Wait for a threaded reply | `npx cyberlegion@0.3.0 mail await --thread <id>` |
| Watch mail as it streams in (observer, never acks) | `npx cyberlegion@0.3.0 mail watch` |
| List addressable peers | `npx cyberlegion@0.3.0 unit who` |
| Sweep dead peers | `npx cyberlegion@0.3.0 unit prune` |
...
| Diagnose the environment (harness, multiplexer, pane, hub root) | `npx cyberlegion@0.3.0 mux doctor` |
| Register the surfacing hook by hand (low-level, explicit harness) | `npx cyberlegion@0.3.0 init --agent <harness>` — the `init-cyberlegion` skill wraps this; use directly only when scripting a known harness |
