...
| Check inbox / read unread mail | `mail inbox`, then `mail read <msg-id>` to peek and `mail ack <msg-id>` once handled |
...
relay through, it spawns the `headless-legate` agent by name, running the same classify-then-route flow,
