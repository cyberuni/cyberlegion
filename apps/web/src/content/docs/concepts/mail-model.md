---
title: Mail Model
description: The mail plane in email terms, covering address vs correlation, delivery, and why the mail plane is deliberately non-authoritative.
---
...
requires registration (an unknown recipient fails to resolve); sending is free, so any label may
...
return](/cyberlegion/concepts/architecture/#delegation-and-return-prefer-wake-over-wait). `mail await --thread
